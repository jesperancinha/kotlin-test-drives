...
class ParadigmDependency1TreeModel : DefaultTreeModel(null), SegmentTreeModel {
    init {
        clear()
    }
    ...
    override fun getDependencies(path: TreePath): List<String> {
        ...
        for (i in 1 until objects.size) {
            subDependencyElement.add((objects[i] as String).intern())
        }
        return subDependencyElement
    }

    override fun addCombination(dependency: Dependency): DefaultMutableTreeNode {
